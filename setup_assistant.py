...
def build_assistant_payload(base_url: str, tools: list[dict]) -> dict:
    ...
    tool_rules = []
    if "search_knowledgebase" in tool_names:
        tool_rules.append("- To answer any question about the documentation: use the search_knowledgebase tool with the user's question as the query")
        tool_rules.append("- If the first search doesn't give a good answer, try rephrasing the query")
    if "list_documents" in tool_names:
        tool_rules.append("- To list what documents are available: use the list_documents tool")
    # Legacy cloud analytics tools
    if "search" in tool_names:
        tool_rules.append("- To find/list resources (apps, datasets, spaces, users): use the search tool")
    if "describe_app" in tool_names:
        tool_rules.append("- To describe an app: use the describe_app tool with the app ID")
    if "get_data_model" in tool_names:
        tool_rules.append("- To get an app's data model: first search to find the app, then use get_data_model with the app ID")
    if "get_fields" in tool_names:
        tool_rules.append("- To get fields in an app: use get_fields with the app ID")
    kb_id = os.getenv("KNOWLEDGEBASE_ID", "")
    if "search_knowledgebase_chunks" in tool_names and kb_id:
    ...
    return {
            ...
            "messages": [
                {
                    "role": "system",
                    "content": (
                        "You are a helpful documentation assistant with access to a knowledge base. "
                        "You can search the documentation and answer questions about MCP Voice architecture, "
                        "setup, customization, troubleshooting, and how the system works.\n\n"
                        ...
                        "ALWAYS use the search_knowledgebase tool to look up answers before responding. "
        ...
        "firstMessage": "Hi, I'm your documentation assistant. Ask me anything about MCP Voice — architecture, setup, troubleshooting, or how it all works.",
